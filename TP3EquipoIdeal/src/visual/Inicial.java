            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    if (cantLideres < 0 || cantArquitectos < 0 || cantProgramadores < 0 || cantTesters < 0) {
                        JOptionPane.showMessageDialog(frmInicial, "Los valores no pueden ser negativos.", "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }

                    PersonalLaboral personal = new PersonalLaboral();

                    IngresoDePersonal ingresoDePersonal = new IngresoDePersonal(
                        personal, cantLideres, cantArquitectos, cantProgramadores, cantTesters
                    );
                    ingresoDePersonal.mostrarVentana();
                    frmInicial.dispose();

                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(frmInicial, "Ingrese solamente números.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
